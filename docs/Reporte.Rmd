---
title: Evaluación causal del efecto de la Guardia Nacional sobre las desapariciones
  en México
author:
...
Este componente resulta particularmente relevante en el presente contexto porque las desapariciones en Guanajuato presentan una tendencia creciente sostenida a lo largo de la muestra. Un modelo que asumiera nivel constante introduciría un sesgo sistemático en el contrafactual; la tendencia local absorbe esa dinámica y produce proyecciones más realistas. La varianza \(\sigma^2_\mu\) controla la velocidad a la que la tendencia puede cambiar: valores pequeños producen una tendencia casi lineal, mientras que valores más grandes le permiten al nivel adaptarse localmente a los datos.

### Selección de covariables mediante prior spike-and-slab

Una de las contribuciones metodológicas centrales del enfoque de Brodersen et al. es el tratamiento bayesiano de la selección de covariables de control. En el presente análisis, el pool inicial consta de 29 entidades federativas, todas potencialmente correlacionadas con Guanajuato durante el pretratamiento. Incluirlas a todas simultáneamente generaría problemas graves de multicolinealidad y sobreajuste; por otro lado, seleccionar manualmente un subconjunto introduciría arbitrariedad en el diseño.

La solución propuesta es asignar a los coeficientes \(\boldsymbol{\beta}\) un prior spike-and-slab (George y McCulloch, 1997), que combina dos distribuciones:

\begin{itemize}
\item \textbf{Spike}: una masa puntual en cero, que representa la hipótesis de que el estado \(j\) no es predictor relevante de Guanajuato. La probabilidad asignada a esta componente controla cuántas variables se espera incluir en el modelo.
\item \textbf{Slab}: una distribución normal de varianza amplia sobre los valores distintos de cero del coeficiente, que describe la magnitud del efecto si la variable sí es relevante.
\end{itemize}

Formalmente, para cada coeficiente \(\beta_j\):

$$
\beta_j \mid \gamma_j \;\sim\; \gamma_j \cdot \mathcal{N}(0,\, v_j \sigma^2) + (1 - \gamma_j) \cdot \delta_0,
$$

donde \(\gamma_j \in \{0,1\}\) es un indicador de inclusión con prior Bernoulli de parámetro \(\pi_j\), y \(\delta_0\) denota la masa puntual en cero. En cada iteración del muestreador, el modelo decide qué entidades tienen coeficiente distinto de cero y cuáles no. La probabilidad posterior de inclusión de cada estado refleja cuánto contribuye a explicar la dinámica pretratamiento de Guanajuato, lo que equivale a una selección automática y cuantificada de las entidades donoras más informativas.

Esta propiedad representa una diferencia conceptual importante respecto al Control Sintético. En el Synth, todos los estados del pool reciben pesos no negativos que suman uno (combinación convexa), y la optimización puede asignar peso cero a algunos, pero no puede asignar pesos negativos ni mayores que uno. El prior spike-and-slab no impone ninguna de estas restricciones: los coeficientes pueden tomar cualquier valor real, permitiendo al modelo capturar relaciones de correlación más generales.

### Estimación e inferencia bayesiana

El modelo se estima exclusivamente sobre el periodo pretratamiento (\(t \leq T_0 =\) mayo de 2019). La estimación se realiza mediante un muestreador de Gibbs que alterna entre dos pasos: primero, se simula el vector de estado latente \(\boldsymbol{\alpha}_{1:T_0}\) condicionado en los parámetros actuales, utilizando el algoritmo de suavizamiento de Kalman; segundo, se simulan los parámetros condicionados en el estado simulado. Este procedimiento genera una secuencia de muestras cuya distribución estacionaria es la distribución posterior conjunta \(p(\boldsymbol{\alpha}, \boldsymbol{\beta}, \sigma^2 \mid y_{1:T_0})\).

En este proyecto se utilizaron 5{,}000 iteraciones MCMC, suficientes para garantizar convergencia estable de las cadenas. Una vez obtenidas las muestras posteriores, el modelo se extrapola hacia el postratamiento (\(t > T_0\)) sin condicionar en los datos observados de Guanajuato, generando la distribución predictiva posterior del contrafactual:

$$
\tilde{Y}_t^{CF} \;\sim\; p\!\left(Y_t \mid y_{1:T_0},\, \mathbf{X}_{1:T}\right), \qquad t > T_0.
$$

El efecto causal puntual en cada periodo se estima como la diferencia entre el valor observado y la media posterior del contrafactual:

$$
\hat{\tau}_t = Y_t^{obs} - \mathbb{E}\!\left[\tilde{Y}_t^{CF}\right],
$$

y los intervalos de credibilidad al 95\% se obtienen directamente de los cuantiles de la distribución posterior de \(\tilde{Y}_t^{CF}\). El efecto acumulado sobre todo el periodo postratamiento se calcula sumando los efectos puntuales periodo a periodo, con su incertidumbre propagada de la misma distribución posterior.

### Supuestos de identificación

Como señalan Brodersen et al. (2015), la validez causal de las inferencias obtenidas por este método descansa sobre un supuesto central: las covariables utilizadas como control no deben haber sido afectadas por la intervención cuyo efecto se desea medir. En el presente análisis, esto equivale a asumir que la implementación de la Guardia Nacional no alteró sistemáticamente la dinámica de desapariciones en las demás entidades utilizadas como predictoras. Este supuesto es más plausible que el de tendencias paralelas globales requerido por el DID, porque no exige que todas las entidades evolucionen en paralelo con Guanajuato, sino únicamente que no hayan sido perturbadas por el mismo tratamiento de manera diferencial.

Un segundo supuesto es que el modelo ajustado sobre el pretratamiento es un buen predictor del contrafactual postratamiento; es decir, que la relación estructural entre Guanajuato y sus covariables no cambió en ausencia del tratamiento. La calidad del ajuste pretratamiento, visible en las figuras de resultados, permite evaluar empíricamente si este supuesto se sostiene.

### Resultados

```{r ci-setup, echo=FALSE, message=FALSE, warning=FALSE}
...
Los cuatro enfoques aplicados en este proyecto responden la misma pregunta causal desde mecanismos de identificación distintos. La Tabla \ref{tab:comparacion-metodos} resume los efectos estimados y los supuestos clave de cada método.

\begin{table}[H]
\centering
\caption{Comparación de métodos causales aplicados al análisis de Guanajuato.}
\label{tab:comparacion-metodos}
\begin{tabular}{p{3.8cm}p{5.0cm}p{4.5cm}}
\toprule
\textbf{Método} & \textbf{Efecto estimado} & \textbf{Supuesto clave} \\
\midrule
DID básico
  & $+543$ desapariciones/periodo
  & Tendencias paralelas globales \\
DID con efectos fijos
  & $+542.6$ des./periodo ($p < 0.001$)
  & Tendencias paralelas + ausencia de shocks específicos \\
Control Sintético
  & Brecha positiva creciente post-2019
  & Contrafactual como combinación convexa de donoras \\
CausalImpact (BSTS)
  & $+$\Sexpr{ef_abs} des./mes (IC: [\Sexpr{ef_abs_lo}, \Sexpr{ef_abs_hi}])
